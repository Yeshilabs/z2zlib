...
const WebRTCComponent: React.FC = () => {
  ...
  const receiveOfferAndAnswer = async () => {
    ...
    const offerResponse = await fetch('/api/signaling?type=offer');
  ...
  const connectToPeer = async () => {
    const answerResponse = await fetch('/api/signaling?type=answer');
    const { answer } = await answerResponse.json();
    if (answer) {
      await peerConnection?.setRemoteDescription(new RTCSessionDescription(answer));
    }

    const candidatesResponse = await fetch('/api/signaling?type=candidates');
